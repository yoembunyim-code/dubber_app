...
# ==========================================
# LICENSE MANAGER (គ្រប់គ្រងការសាក 3 វីដេអូ)
# ==========================================
def load_license():
...
st.title("🎬 AI Khmer Dubbing PRO (Streamlit Edition)")
st.markdown("---")
...
with col1:
    ...
    if 'process_start' in st.session_state and st.session_state['process_start']:
        ...
        # ----------------------------------------------------------------
        # ក្លែងធ្វើដំណើរការ Dubbing តាមជំហាន
        # ----------------------------------------------------------------
        
        log_text += "[80%] Aligning audio... / កំពុងដកស្រង់សំឡេងដើម...\n"
        ...
        if st.session_state['process_start']:
            log_text += "[85%] Translating to Khmer... / កំពុងបកប្រែអក្សរជាខ្មែរ...\n"
            log_area.code(log_text)
            progress_bar.progress(0.85)
            time.sleep(1.5)

        if st.session_state['process_start']:
            log_text += "[90%] Generating Khmer Voice (TTS)... / កំពុងបង្កើតសំឡេងខ្មែរ...\n"
            log_area.code(log_text)
            progress_bar.progress(0.90)
            time.sleep(2)

        if st.session_state['process_start']:
            log_text += "[96%] Mixing subtitle & audio into video... / កំពុងផ្សំសំឡេងចូលវីដេអូ...\n"
            log_area.code(log_text)
